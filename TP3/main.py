...
def delete_EEPROM():
    try:
        s = get_serial()
        if s is not None:
            # envia marcador 'B' (Corregido el marcador interno)
            ser.write(b'B\n')
            shareMem["events"] = []
...
def listening_serial_port():
    while True:
        try:
            ...
            if line:
                print("Arduino mandó:", line)

            if line and ',' in line:
                # puerto serial envia: evento,tiempo\n
                line_aux = line.split(",")            
                event = line_aux[0].strip()
                
                try:
                    timestamp_crudo = int(line_aux[1].strip())
                    if event != "255" and timestamp_crudo > 0:
                        res_time = time.ctime(timestamp_crudo)
                        shareMem["events"].append({
                            "evento": event, 
                            "hora": res_time
                        })
                except Exception as e: 
                    print(f"Dato corrupto ignorado: {line} - Motivo: {e}")
                    pass
